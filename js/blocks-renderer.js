...
(function () {
  ...
  const RENDERERS = {
    ...
    gallery_preview(block, ctx) {
      const items = ((ctx && ctx.galleryItems) || []).slice(0, block.limit || 6);
      const imgs = items
  ...
  async function renderBlocks(jsonUrl, mountSelector) {
    ...
    try {
      ...
      // Beberapa tipe block (mis. gallery_preview) butuh data dari file lain.
      // Diambil sekali di awal supaya tidak fetch berulang per block.
