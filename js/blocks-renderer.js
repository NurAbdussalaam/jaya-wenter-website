...
(function () {
  ...
  const RENDERERS = {
    ...
    gallery_preview(block, ctx) {
      const allItems = ((ctx && ctx.galleryItems) || []);

      // Mode 1: pilih manual (field "pilih" = array nama file image)
      // Mode 2: fallback otomatis (ambil "limit" foto terbaru)
      let items;
      if (Array.isArray(block.pilih) && block.pilih.length > 0) {
        const map = new Map(allItems.map((it) => [it.image, it]));
        items = block.pilih.map((name) => map.get(name)).filter(Boolean);
      } else {
        items = allItems.slice(0, block.limit || 6);
      }

      const imgs = items
        .map((it) => `<img src="galeri/${esc(it.image)}" alt="${esc(it.alt || it.title || "")}">`)
        .join("");
  ...
  async function renderBlocks(jsonUrl, mountSelector) {
    ...
    try {
      ...
      if (blocks.some((b) => b.type === "gallery_preview")) {
        try {
          const gRes = await fetch("content/gallery.json", { cache: "no-store" });
          const gData = await gRes.json();
          galleryItems = gData.items || [];
        } catch (err) {
          console.error("Gagal memuat content/gallery.json:", err);
          galleryItems = [];
        }
      }
      const ctx = { galleryItems };
