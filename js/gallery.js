/* ======================================================= */
/* GALLERY.JS                                              */
/* Membaca data dari /content/gallery.json dan menampilkan */
/* semua item gallery + filter pencarian + lightbox.       */
/* ======================================================= */

(function () {
  // Path ke file data gallery (sesuaikan jika struktur folder berbeda)
  const GALLERY_JSON_PATH = "/content/gallery.json";

  // Setelah DOM siap, mulai fetch data
  document.addEventListener("DOMContentLoaded", function () {
    fetch(GALLERY_JSON_PATH)
      ...
      .catch(function (err) {
        ...
        if (grid) {
          grid.innerHTML =
            '<p style="color:#c00;text-align:center;">Gagal memuat data gallery.</p>';
        }
      });
  });

  /* ---------------------------------------------------- */
  /* Render semua item gallery ke dalam #galleryGrid      */
  /* ---------------------------------------------------- */
  function renderGallery(items) {
    ...
    items.forEach(function (item) {
      ...
      el.innerHTML =
        '<img src="' + item.image + '" alt="' + (item.alt || item.title) + '" onclick="openLightbox(this.src)">' +
  ...
  /* ---------------------------------------------------- */
  /* Filter pencarian (sama persis logika HTML asli)      */
  /* ---------------------------------------------------- */
  function initializeFilters() {
    ...
    function wholeWordMatch(haystack, word) {
      if (!word) return true;
      var pattern = new RegExp("\\b" + escapeRegex(word) + "\\b", "i");
    ...
    function parseQuery(raw) {
      ...
      for (var i = 0; i < colorPhrases.length; i++) {
        var phrase = colorPhrases[i][0];
        var slug = colorPhrases[i][1];
      ...
      for (var j = 0; j < keys.length; j++) {
        var key = keys[j];
        var re2 = new RegExp("\\b" + escapeRegex(key) + "\\b", "i");
        if (re2.test(text)) {
          kategori = categoryKeywords[key];
          text = text.replace(re2, " ").replace(/\s+/g, " ").trim();
          break;
        }
      }

      var freeWords = text.split(" ").filter(Boolean);
      return { kategori: kategori, warna: warna, freeWords: freeWords };
    }
    ...
    function applyFilters() {
      ...
      items.forEach(function (item) {
        var dataKategori = item.dataset.kategori;
        var dataWarna = item.dataset.warna;
        ...
        if (finalKategori && dataKategori !== finalKategori) match = false;
        if (finalWarna && dataWarna !== finalWarna) match = false;

        if (match && parsed.freeWords.length) {
          match = parsed.freeWords.every(function (word) {
            return wholeWordMatch(haystack, word);
          });
        }
      ...
      if (resultCountEl) {
        resultCountEl.textContent = isFiltering ? visibleCount + " hasil ditemukan" : "";
      }
      if (noResultMsg) {
        noResultMsg.style.display = isFiltering && visibleCount === 0 ? "block" : "none";
      }
      if (searchClearBtn) {
    ...
    kategoriButtons.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (activeKategori === val) {
          activeKategori = null;
          btn.classList.remove("active");
        } else {
          activeKategori = val;
          kategoriButtons.forEach(function (b) {
    ...
    warnaButtons.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (activeWarna === val) {
          ...
          activeWarna = val;
          warnaButtons.forEach(function (b) {
            b.classList.remove("active");
          });
          btn.classList.add("active");
        }
